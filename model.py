...
def _model_train(df, tag, test=False):
    ...
    if test:
        ...
        subset_indices = np.random.choice(np.arange(X.shape[0]),
                                          n_samples, replace=False).astype(int)
    ...
    param_grid_rf = {'rf__criterion': ['mse', 'mae'],
                     'rf__n_estimators': [10, 15, 20, 25]}

    pipe_rf = Pipeline(steps=[('scaler', StandardScaler()), ('rf',
                       RandomForestRegressor())])

    # # grid = GridSearchCV(pipe_rf, param_grid=param_grid_rf, cv=5, iid=False, n_jobs=-1)

    grid = GridSearchCV(pipe_rf, param_grid=param_grid_rf, cv=5,
                        n_jobs=-1)
    ...
    if test:
        saved_model = os.path.join(MODEL_DIR,
                                   'test-{}-{}.joblib'.format(tag,
                                                              model_name))
        print('... saving test version of model: {}'.format(saved_model))
    else:
        saved_model = os.path.join(MODEL_DIR,
                                   'sl-{}-{}.joblib'.format(tag,
                                                            model_name))
...
def model_load(prefix='sl', data_dir=None, training=True):
    ...
    models = [f for f in os.listdir(os.path.join('.', 'models'))
              if re.search('sl', f)]
    ...
    for model in models:
        ...
        all_models[re.split('-', model)[1]] = \
            joblib.load(os.path.join('.', 'models', model))
...
def model_predict(
    ...
    target_date = '{}-{}-{}'.format(year, str(month).zfill(2),
                                    str(day).zfill(2))
    print(target_date)
