...
def fetch_data(data_dir):
    ...
    file_list = [os.path.join(data_dir, f) for f in os.listdir(
        data_dir) if re.search("\.json", f)]
    ...
    for f, df in all_months.items():
        ...
        if sorted(cols) != correct_columns:
            raise Exception(
                "columns name could not be matched to correct cols")

    # concat all of the data
    df = pd.concat(list(all_months.values()), sort=True)
    years, months, days = df['year'].values, df['month'].values, df['day'].values
    dates = ["{}-{}-{}".format(years[i], str(months[i]).zfill(2),
                               str(days[i]).zfill(2)) for i in range(df.shape[0])]
...
def convert_to_ts(df_orig, country=None):
    ...
    start_month = '{}-{}'.format(df['year'].values[0],
                                 str(df['month'].values[0]).zfill(2))
    stop_month = '{}-{}'.format(df['year'].values[-1],
    ...
    streams = [np.unique(df[df_dates == day]
                         ['stream_id'].values).size for day in days]
...
def fetch_ts(data_dir, clean=False):
    ...
    countries = [os.path.join(
        data_dir, "ts-"+re.sub("\s+", "_", c.lower()) + ".csv") for c in top_ten_countries]
    ...
    for d, day in enumerate(dates):
        ...
        for num in previous:
            ...
            mask = np.in1d(dates, np.arange(
                prev, current, dtype='datetime64[D]'))
